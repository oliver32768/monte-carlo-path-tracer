...
void quads(std::ofstream& img, int spp)
{
    ...
    world.add(make_shared<triangle>(point3(-2,-2, 0), vec3(4, -1, 0), vec3(2, 4, 0), back_green));
...
void final_scene(int image_width, int spp, int max_depth, std::ofstream& img)
{
    ...
    // Grey fuzzy metal sphere
    world.add(make_shared<sphere>(
            point3(0, 150, 145), 50, make_shared<metal>(color(0.8, 0.8, 0.9), 1.0)
    ));

    // Blue metallic sphere with subsurface scattering
    auto boundary = make_shared<sphere>(point3(360,150,145), 70, make_shared<dielectric>(1.5));
    world.add(boundary);
    world.add(make_shared<constant_medium>(boundary, 0.2, color(0.2, 0.4, 0.9)));

    // World medium to scatter rays travelling to the background into emitter
    ...
    // Translate and rotate said boxes, then build BVH over them
